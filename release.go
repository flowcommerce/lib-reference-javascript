...
func main() {
	...
	executor = executor.Add("git pull")
	executor = executor.Add("cd lib/javascript && npm --no-git-tag-version version patch")
	executor = executor.Add("git add lib/javascript")
	executor = executor.Add("git commit -m 'autocommit: increment version' lib/javascript")
